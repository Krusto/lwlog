...
namespace lwlog::details
{
    ...
    memory_buffer<Capacity>::memory_buffer() : m_buffer{m_inline_buffer}, m_capacity{Capacity}
    {}
    ...
    void memory_buffer<Capacity>::grow(std::size_t new_capacity)
    {
        ...
        if (m_size > 0) { std::memcpy(new_buffer, m_buffer, m_size); }
    ...
    {
        if (m_size + size > m_capacity) { memory_buffer<Capacity>::grow(m_capacity * 1.5f); }
    ...
    {
        if (m_size + 1 > m_capacity) { memory_buffer<Capacity>::grow(m_capacity * 1.5f); }
    ...
    {
        if (m_size + insert_size > m_capacity) { memory_buffer<Capacity>::grow((size_t) ((float) m_capacity * 1.5f)); }
    ...
    {
        if (m_size < m_capacity) { m_buffer[m_size] = '\0'; }
    ...
    void convert_to_chars(char* const __restrict buffer, std::size_t buffer_size, const T& value)
    {
        ...
        if constexpr (std::is_same_v<T, bool>)
        {
            ...
            if (value_size > max_write) { value_size = max_write; }
