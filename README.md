...
```{path}``` | ```%@``` | Path of the file in which the log function is called | "C:\Users\user\Desktop\lwlog\Sandbox\Sandbox.cpp"
```{file}``` | ```%$``` | Name of the file in which the log function is called | "Sandbox.cpp"
